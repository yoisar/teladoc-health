...
const addNumbers = (firstString: string, secondString: string) => {
    ...
    try {
         ...
         */
        let listA: number[] = firstString.trim().split(' ').map(str => Number(str));
        let listB: number[] = secondString.trim().split(' ').map(str => Number(str));
        /**
        ...
        if (listA.length == listB.length) {
            ...
            let sumResulInt: BigInt;
            //save the float result of pair sum
            let sumResulFloat: number;
